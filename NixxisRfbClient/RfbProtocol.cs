...
namespace Nixxis.RfbClient
{
	...
	public class RfbProtocol
	{
		...
		public sealed class ZRLECompressedReader : BinaryReader
		{
			...
			public ZRLECompressedReader(Stream compressedStream) : base(compressedStream)
			{
			...
			public void DecodeStream()
			{
                ...
                using (MemoryStream ms = new MemoryStream())
                {
                    ...
                    {
                        ms.WriteByte((byte)BaseStream.ReadByte());
                    }

                    ms.Position = 0;

                    using (var hgs = new GZipStream(ms, CompressionMode.Decompress, true))
                    {
                        while (true)
                        {
                            byte[] bu = new byte[1024];
                            int read = hgs.Read(bu, 0, 1024);
                            zlibMemoryStream.Write(bu, 0, read );
                            if( read < 1024)
                            {                                
                                break;
                            }
