			...
			try {
				...
				if (types.Length > 0) {
					if (types[0] == 0) {
						...
						throw new InvalidOperationException("Connection Failed. The server rejected the connection: " + rfb.ReadSecurityFailureReason());
					} else {
						...
						if (rfb.ServerVersion == 3.8f && securityType == 1) {
							if (rfb.ReadSecurityResult() > 0) {
                                ...
                                throw new InvalidOperationException("Unable to Connecto to the Server. The Server rejected the connection: " + rfb.ReadSecurityFailureReason());
							}
						}
                    ...
                    throw new InvalidOperationException("Protocol Error Connecting to Server. The Server didn't send any Security Types during the initial handshake.");
				}
			} catch (Exception e) {
                throw new InvalidOperationException("Unable to connect to the server. Error was: " + e.Message, e);
			}			
		}
...
//					|| types[i] == 18 	// TLS
		...
		public void Disconnect()
		{
			...
			worker.Join(100);	// this number is arbitrary, just so that it doesn't block forever....
                    ...
                    switch (rfb.ReadServerMessageType()) {
                        case RfbProtocol.FRAMEBUFFER_UPDATE:
                            ...
                            for (int i = 0; i < rectangles; ++i) {
                                ...
                                System.Diagnostics.Trace.WriteLine(string.Format("Updating rect at {0}", er.UpdateRectangle));
