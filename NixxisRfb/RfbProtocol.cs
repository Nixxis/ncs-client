    ...
    public class RfbProtocol
    {
        ...
        public void Start()
        {
            ...
            try
            {
                ...
                System.Diagnostics.Trace.WriteLine(string.Format("Listening started"), "VNCServer");
            }
        ...
        public void Close()
        {
            ...
            try
            {
                ...
                {
                    serverSocket.Stop();
                    System.Diagnostics.Trace.WriteLine(string.Format("Listening stopped"), "VNCServer");
                    serverSocket = null;
                }
                localClient.Close();
//                localClient.Disconnect(true);
        ...
        private int[] LastRectBackup = null;
        ...
        private void DoFrameBufferUpdate(Framebuffer fb, bool incremental, int x, int y, int width, int height)
        {
            ...
            try
            {
                ...
                EncodedRectangle[] diffs = null;

                int[] backup = localRect.GetCopy(); 
                if(incremental)
                {
                    System.Diagnostics.Trace.WriteLine("Incremental requested");
                    if (LastRectBackup != null )
                    {
                        diffs = localRect.ComputeChanges(LastRectBackup);
                    }
                }

                LastRectBackup = backup;

                if (diffs != null)
                {
                    foreach (EncodedRectangle er in diffs)
                    {
                        er.Encode();
                        lst.Add(er);
                    }
