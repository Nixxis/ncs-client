    ...
    public class PropertyItemValue : INotifyPropertyChanged
    {
        ...
        public PropertyItemValue(PropertyItem property)
        {
            ...
            if (_hasSubProperties)
            {
                ...
                var propertyAttributes = PropertyGridUtils.GetAttributes<BrowsablePropertyAttribute>(value);

                PropertyDescriptorCollection descriptors = property.Converter.GetProperties(value);
                List<BrowsablePropertyAttribute> temp = new List<BrowsablePropertyAttribute>(propertyAttributes);

                foreach (PropertyDescriptor d in descriptors)
                {
                    if (ShoudDisplayProperty(d, temp))
                    {
        ...
        private bool ShoudDisplayProperty(PropertyDescriptor propertyDescriptor, List<BrowsablePropertyAttribute> browsableAttributes)
        {
            ...
            var attribute = browsableAttributes.FirstOrDefault(item => item.PropertyName == propertyDescriptor.Name);
            if (attribute != null) return attribute.Browsable;

            // Check the wildcard
            var wildcard = browsableAttributes.FirstOrDefault(item => item.PropertyName == BrowsablePropertyAttribute.All);
        ...
        public bool IsMultiLines
        {
            get { return _property.IsMultiLines; }
        }
